...
u = Sheets$Residual$`Bond Returns`
v = Sheets$Residual$`Oil Returns`
date = Sheets$Residual$Period
...
write.table(dcc.results$out,file="dccparam_lag0_Residual.txt");
...
for (k in 1:M) {
  ...
  out=dcc.results$out;
  # Output Parameter estimates and their robust standard errors??   dcc.results$out
  write.table(out,file="dccparam_positivelag_Residual.txt",append=TRUE);
...
for (k in 1:M) {
  ...
  out=dcc.results$out;
  # Output Parameter estimates and their robust standard errors??   dcc.results$out
  write.table(out,file="dccparam_negativelag_Residual.txt",append=TRUE);
...
write.csv(outHong, file="mgarch_Hong_Granger_Residual.csv", sep = " ");
