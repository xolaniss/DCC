...
Sheets[[2]][1] <- Sheets[[2]][1] %>%  map (date_replace1)
Sheets[[2]][1] <- Sheets[[2]][1] %>%  map (date_replace2)

Sheets$Volatility$Period <- paste(Sheets$Volatility$Period,"-01", sep = "")
Sheets$Volatility$Period <- as.yearmon(as.Date(Sheets$Volatility$Period, format = "%Y-%m-%d"))

#Defining Variables ----

u = Sheets$Volatility$`Bond Returns Volatility`
v = Sheets$Volatility$`Oil Returns Volatility`
date = Sheets$Volatility$Period
...
write.table(dcc.results$out,file="dccparam_lag0_Volatility.txt");
...
for (k in 1:M) {
  ...
  out=dcc.results$out;
  # Output Parameter estimates and their robust standard errors??   dcc.results$out
  write.table(out,file="dccparam_positivelag_Volatility.txt",append=TRUE);
...
for (k in 1:M) {
  ...
  out=dcc.results$out;
  # Output Parameter estimates and their robust standard errors??   dcc.results$out
  write.table(out,file="dccparam_negativelag_Volatility.txt",append=TRUE);
...
write.csv(outHong, file="mgarch_Hong_Granger_Volatility.csv", sep = " ");
