...
write.table(dcc.results$out,file="dccparam_lag0_Bond2.txt");
...
for (k in 1:M) {
  ...
  out=dcc.results$out;
  # Output Parameter estimates and their robust standard errors??   dcc.results$out
  write.table(out,file="dccparam_positivelag_Bond2.txt",append=TRUE);
...
for (k in 1:M) {
  ...
  out=dcc.results$out;
  # Output Parameter estimates and their robust standard errors??   dcc.results$out
  write.table(out,file="dccparam_negativelag_Bond2.txt",append=TRUE);
...
write.csv(outHong, file="mgarch_Hong_Granger_Bond2.csv", sep = " ");

#Importing
results <- read.csv("mgarch_Hong_Granger_Bond2.csv")
...
Bonds2$date <- Sheets$Bonds2$Period[-c(1:7)]
...
Bonds2$ph1 <- discrete(Bonds2$ph1)
Bonds2$ph2<- discrete(Bonds2$ph2)
Bonds2$ph10<- discrete(Bonds2$ph10)
Bonds2$ph20<- discrete(Bonds2$ph20)
Bonds2$phb<- discrete(Bonds2$phb)

#Exporting

write.csv(Bonds2, file="Bonds2.csv", row.names = F);
...
rug_plot(Bonds2, y.var = H1, z.var = ph1, plot.name = "Bonds2_ph1")
rug_plot(Bonds2, y.var = H2, z.var = ph2, plot.name = "Bonds2_ph2")
rug_plot(Bonds2, y.var = H10, z.var = ph10, plot.name = "Bonds2_ph10")
rug_plot(Bonds2, y.var = H20, z.var = ph20, plot.name = "Bonds2_ph20")
rug_plot(Bonds2, y.var = Hb, z.var = phb, plot.name = "Bonds2_phb")
