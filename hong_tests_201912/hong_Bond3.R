...
v = Sheets$Bonds3$`Oil Returns`
date = Sheets$Bonds3$Period
...
write.table(dcc.results$out,file="dccparam_lag0_Bond3.txt");
...
for (k in 1:M) {
  ...
  out=dcc.results$out;
  # Output Parameter estimates and their robust standard errors??   dcc.results$out
  write.table(out,file="dccparam_positivelag_Bond3.txt",append=TRUE);
...
for (k in 1:M) {
  ...
  out=dcc.results$out;
  # Output Parameter estimates and their robust standard errors??   dcc.results$out
  write.table(out,file="dccparam_negativelag_Bond3.txt",append=TRUE);
...
write.csv(outHong, file="mgarch_Hong_Granger_Bond3.csv", sep = " ");
...
write.csv(Bonds3, file="Bonds3.csv", row.names = F);
