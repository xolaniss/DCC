...
SP500_returns <-  data[, 3]
monthly_dates <- as.Date(data[, 1], "%Y-%m-%d")
...
write.table(dcc.results$out,file="dccparam_lag0_dollar_pound_monthly.txt");
...
for (k in 1:M) {
  ...
  out=dcc.results$out;
  # Output Parameter estimates and their robust standard errors??   dcc.results$out
  write.table(out,file="dccparam_positivelag_dollar_pound_monthly.txt",append=TRUE);
...
for (k in 1:M) {
  ...
  out=dcc.results$out;
  # Output Parameter estimates and their robust standard errors??   dcc.results$out
  write.table(out,file="dccparam_negativelag_dollar_pound_monthly.txt",append=TRUE);
...
write.csv(outHong, file="mgarch_Hong_Granger_dollar_pound_monthly.csv", sep = " ");


#Importing
results <- read.csv("mgarch_Hong_Granger_dollar_pound_monthly.csv")

#Renaming ----

results[, 1] <- NULL
colnames(results) <- c("date","H1","ph1","H2","ph2","H10","ph10","H20","ph20","Hb","phb")
dollar_pound_monthly <- results

#Merging dates for graphing

dollar_pound_monthly$date <- as.yearmon(monthly_dates[-c(1:4)],  format = "%Y-%m-%d")
...
dollar_pound_monthly$ph1 <- discrete(dollar_pound_monthly$ph1)
dollar_pound_monthly$ph2<- discrete(dollar_pound_monthly$ph2)
dollar_pound_monthly$ph10<- discrete(dollar_pound_monthly$ph10)
dollar_pound_monthly$ph20<- discrete(dollar_pound_monthly$ph20)
dollar_pound_monthly$phb<- discrete(dollar_pound_monthly$phb)
#Exporting 

write.csv(dollar_pound_monthly, file="dollar_pound_monthly.csv", row.names = F);
...
rug_plot(dollar_pound_monthly, y.var = H1, z.var = ph1, plot.name = "dollar_pound_monthly_ph1")
rug_plot(dollar_pound_monthly, y.var = H2, z.var = ph2, plot.name = "dollar_pound_monthly_ph2")
rug_plot(dollar_pound_monthly, y.var = H10, z.var = ph10, plot.name = "dollar_pound_monthly_ph10")
rug_plot(dollar_pound_monthly, y.var = H20, z.var = ph20, plot.name = "dollar_pound_monthly_ph20")
rug_plot(dollar_pound_monthly, y.var = Hb, z.var = phb, plot.name = "dollar_pound_monthly_phb")
