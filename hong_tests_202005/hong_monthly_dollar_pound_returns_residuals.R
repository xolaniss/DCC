...
monthly_dates_residuals <- as.Date(data[, 1], "%Y-%m-%d")
...
write.table(dcc.results$out,file="dccparam_lag0_dollar_pound_monthly_residuals.txt");
...
for (k in 1:M) {
  ...
  out=dcc.results$out;
  # Output Parameter estimates and their robust standard errors??   dcc.results$out
  write.table(out,file="dccparam_positivelag_dollar_pound_monthly_residuals.txt",append=TRUE);
...
for (k in 1:M) {
  ...
  out=dcc.results$out;
  # Output Parameter estimates and their robust standard errors??   dcc.results$out
  write.table(out,file="dccparam_negativelag_dollar_pound_monthly_residuals.txt",append=TRUE);
...
write.csv(outHong, file="mgarch_Hong_Granger_dollar_pound_monthly_residuals.csv", sep = " ");
