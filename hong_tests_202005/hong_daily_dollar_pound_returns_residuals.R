...
dollar_pound_volatility_residuals <- data[, 4]
SP500_volatility_residuals <-  data[, 5]
daily_dates_residuals <- as.Date(data[, 1], "%Y-%m-%d")

#Defining Variables ----
u = dollar_pound_volatility_residuals
v = SP500_volatility_residuals
date = daily_dates_residuals
...
write.table(dcc.results$out,file="dccparam_lag0_dollar_pound_daily_residuals.txt");
...
for (k in 1:M) {
  ...
  out=dcc.results$out;
  # Output Parameter estimates and their robust standard errors??   dcc.results$out
  write.table(out,file="dccparam_positivelag_dollar_pound_daily_residuals.txt",append=TRUE);
...
for (k in 1:M) {
  ...
  out=dcc.results$out;
  # Output Parameter estimates and their robust standard errors??   dcc.results$out
  write.table(out,file="dccparam_negativelag_dollar_pound_daily_residuals.txt",append=TRUE);
...
write.csv(outHong, file="mgarch_Hong_Granger_dollar_pound_daily_residuals.csv", sep = " ");


#Importing
results <- read.csv("mgarch_Hong_Granger_dollar_pound_daily_residuals.csv")

#Renaming ----

results[, 1] <- NULL
colnames(results) <- c("date","H1","ph1","H2","ph2","H10","ph10","H20","ph20","Hb","phb")
dollar_pound_daily_residuals <- results

#Merging dates for graphing

#dollar_pound_daily$date <- as.Date(daily_dates[-4], "%Y-%m-%d")
dollar_pound_daily_residuals$date <-as.yearmon(daily_dates_residuals[-c(1:4)], format = "%Y-%m-%d")
# Discrete p values
discrete <- function(pvalue) {
  case_when(pvalue < 0.05 ~ "1", pvalue > 0.05 ~ "0", TRUE ~ "Other")
}

dollar_pound_daily_residuals$ph1 <- discrete(dollar_pound_daily_residuals$ph1)
dollar_pound_daily_residuals$ph2<- discrete(dollar_pound_daily_residuals$ph2)
dollar_pound_daily_residuals$ph10<- discrete(dollar_pound_daily_residuals$ph10)
dollar_pound_daily_residuals$ph20<- discrete(dollar_pound_daily_residuals$ph20)
dollar_pound_daily_residuals$phb<- discrete(dollar_pound_daily_residuals$phb)
#Exporting 

write.csv(dollar_pound_daily_residuals, file="dollar_pound_daily_residuals.csv", row.names = F);
...
rug_plot(dollar_pound_daily_residuals, y.var = H1, z.var = ph1, plot.name = "dollar_pound_daily_residuals_ph1")
rug_plot(dollar_pound_daily_residuals, y.var = H2, z.var = ph2, plot.name = "dollar_pound_daily_residuals_ph2")
rug_plot(dollar_pound_daily_residuals, y.var = H10, z.var = ph10, plot.name = "dollar_pound_daily_residuals_ph10")
rug_plot(dollar_pound_daily_residuals, y.var = H20, z.var = ph20, plot.name = "dollar_pound_daily_residuals_ph20")
rug_plot(dollar_pound_daily_residuals, y.var = Hb, z.var = phb, plot.name = "dollar_pound_daily_residuals_phb")
